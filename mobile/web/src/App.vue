...
function readSettings(){
  ...
  try{
    ...
    return {...defaults,...saved,layoutRevision:2};
...
watch([()=>settings.theme,()=>settings.font,()=>settings.accent,()=>settings.ptt,systemDark],()=>{localStorage.setItem('hxz-mobile-settings',JSON.stringify(settings));document.documentElement.dataset.theme=dark.value?'dark':'light';document.documentElement.style.fontSize=Math.max(14,Math.min(22,Number(settings.font)||16))+'px';document.documentElement.style.setProperty('--accent',settings.accent);if(window.HXZNative)native('appearance',{dark:dark.value}).catch(()=>{});},{immediate:true});
...
watch(tab,async(next,previous)=>{
  ...
  if(settings.animations&&!matchMedia('(prefers-reduced-motion: reduce)').matches&&workspace.value?.animate){
    ...
    pageAnimation=workspace.value.animate([{opacity:.15,transform:'translateX('+direction*7+'px)'},{opacity:1,transform:'translateX(0)'}],{duration:160,easing:'cubic-bezier(.16,1,.3,1)'});
  }
});
...
unsubs.push(on('message',()=>{if(tab.value==='chat'&&nearBottom.value)nextTick(scrollBottom);else newMessages.value++;}));
watch(()=>state.messages.length,()=>{if(nearBottom.value&&tab.value==='chat')nextTick(scrollBottom);});
...
function insertEmoji(value,target){if(target==='reply')replyBody.value+=value;else chatBody.value+=value;nextTick(()=>composer.value?.focus());}
...
const posts=ref([]),postTotal=ref(0),postOffset=ref(0),postQuery=ref(''),postCategory=ref(''),detail=ref(null),replyBody=ref(''),replyParent=ref(null),replyOffset=ref(0);
...
async function sendReply(){await run(async()=>{await api('/api/forum/posts/'+detail.value.post.id+'/replies','POST',{body:replyBody.value,parentId:replyParent.value?.id||null});replyBody.value='';replyParent.value=null;replyOffset.value=Math.floor(detail.value.total/24)*24;await loadReplies();notify('回复已发布');});}
...
<div class="app-shell" :aria-hidden="overlayCount>0?true:undefined" :inert="overlayCount>0?true:undefined" @invalid.capture.prevent="invalidField">
  ...
  <main ref="workspace" class="workspace" :aria-hidden="overlayCount>0?true:undefined" :inert="overlayCount>0?true:undefined">
    <section v-show="tab==='chat'" class="chat-page">
      ...
      <div ref="chatList" class="message-list" @scroll="chatScroll">
        <div v-if="!state.messages.length" class="empty-state"><Icon name="chat"/><h2>{{state.connected?'在这里相遇':'欢迎来到幻想镇'}}</h2><p>{{state.connected?'向在线的旅人打个招呼吧':'登录皮肤站账号，加入聊天与语音'}}</p><button v-if="!state.hasAccount" class="primary" @click="sheet='login'">登录社区</button><button v-else-if="!state.connected" class="soft-button" @click="run(()=>native('reconnect'))">重新连接</button></div>
        <article v-for="(m,index) in state.messages" :key="m.id" class="message" :class="{own:m.uid===state.user.uid,grouped:index>0&&state.messages[index-1].uid===m.uid&&m.created-state.messages[index-1].created<120000}">
    ...
    <section v-if="tab==='forum'" class="content-page">
      ...
      <template v-else><button class="text-button back" @click="detail=null"><Icon name="back"/>论坛</button><article class="detail-card"><span class="tag">{{detail.post.category}}</span><h1>{{detail.post.title}}</h1><p class="meta">{{detail.post.name}} · {{date(detail.post.created)}}</p><p class="prose">{{detail.post.body}}</p><div class="actions"><button :class="['soft-button',{selected:detail.post.liked}]" :disabled="!state.connected||busy" @click="likePost"><Icon name="like"/>{{detail.post.likes}}</button><button v-if="access.admin||detail.post.uid===state.user.uid" class="text-button danger-text" @click="deletePost">删除</button><template v-if="access.admin"><button class="text-button" @click="moderate('pinned')">{{detail.post.pinned?'取消置顶':'置顶'}}</button><button class="text-button" @click="moderate('locked')">{{detail.post.locked?'解锁':'锁帖'}}</button><button class="text-button" @click="moderate('hidden')">隐藏</button></template></div></article>
        ...
        <form v-if="state.connected&&!detail.post.locked" class="reply-compose card" @submit.prevent="sendReply"><div v-if="replyParent" class="reply-target"><span>回复 {{replyParent.name}}</span><button type="button" class="icon-button" aria-label="取消指定回复" @click="replyParent=null"><Icon name="close"/></button></div><textarea v-model="replyBody" placeholder="写下你的回复…" maxlength="5000" rows="3" required/><div class="emoji-tray inline"><button v-for="e in emojis.slice(0,6)" :key="e" type="button" @click="insertEmoji(e,'reply')">{{e}}</button></div><button class="primary" :disabled="busy||!replyBody.trim()">发布回复</button></form><p v-else class="empty-note">{{detail.post.locked?'该帖子已锁定':'登录后参与讨论'}}</p>
      </template>
    </section>
