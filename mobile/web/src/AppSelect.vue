...
<template>
  ...
  <AppOverlay :open="open" :label="label" kind="picker" @close="open=false">
    <section class="sheet"><div class="sheet-handle"/><header class="sheet-header"><h2>{{label}}</h2><button type="button" class="icon-button" aria-label="关闭选项" @click="open=false"><Icon name="close"/></button></header>
      <div class="sheet-body"><input v-if="items.length>12" v-model="filter" class="choice-search" type="search" placeholder="搜索选项" aria-label="搜索选项"><div ref="list" @keydown="arrows"><ChoiceRows :items="visible" :value="modelValue" :label="label" @choose="choose"/><p v-if="!visible.length" class="empty-note">没有匹配的选项</p></div></div>
    </section>
  </AppOverlay>
</template>
