...
run('javac',['-encoding','UTF-8','--release','8','-Xlint:-options','-cp',jar,'-d',out,path.join(root,'client/installer-java/up/hxz/Network.java'),path.join(root,'client/test/java/NetworkCleanupRegression.java')]);
run('java',['-cp',out+path.delimiter+jar,'up.hxz.NetworkCleanupRegression',out,...process.argv.slice(2)]);
