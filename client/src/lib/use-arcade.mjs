...
export function useArcade(request){
 const selected=ref(null),canvas=ref(),game=shallowRef(null),score=ref(0),playing=ref(false),paused=ref(false),busy=ref(false),message=ref(''),ranking=ref([]),rankSelf=ref(null),rankLabel=ref(''),rankError=ref(''),ranked=ref(false),finished=ref(false),countdown=ref(0),elapsed=ref(0),level=ref(1),lines=ref(0),sound=ref(false),pendingScore=ref(null);
 let saved={};try{saved=JSON.parse(localStorage.getItem('hxz-arcade-best')||'{}');}catch{}
 const rankPeriod=ref('week'),syncPaused=ref(false);
 ...
 async function loadRanking(){if(!selected.value||document.hidden||closed||loadingRanking)return;const id=selected.value.id;loadingRanking=true;try{const data=await request('/api/arcade/'+id+'?period='+rankPeriod.value);if(!closed&&selected.value?.id===id){ranking.value=data.items;rankSelf.value=data.self;rankLabel.value=data.label||'';rankError.value='';}}catch{if(!closed&&selected.value?.id===id)rankError.value='暂时无法获取排名。社区服务端需升级至 0.5.0。';}finally{loadingRanking=false;if(!closed&&selected.value&&selected.value.id!==id)void loadRanking();}}
 ...
 async function start(){if(busy.value||playing.value||submitting||checkpointPromise)return;busy.value=true;message.value='';finished.value=false;pendingScore.value=null;prepareAudio();
 ...
 function loop(now){if(!playing.value)return;const dt=clock?Math.min(.12,(now-clock)/1000):0;clock=now;
  if(!paused.value){if(countdownLeft>0){countdownLeft=Math.max(0,countdownLeft-dt);countdown.value=Math.ceil(countdownLeft);}else{accumulator+=dt;while(accumulator>=1/30&&playing.value){accumulator-=1/30;const s=game.value;
   ...
   const actions=queued.splice(0,4);if(ranked.value)for(const action of actions)inputs.push([s.tick+1,action]);s.renderPrevious={y:s.y,ballX:s.ball?.x,ballY:s.ball?.y,paddle:s.paddle};
 ...
 function pause(){if(!playing.value)return;if(syncPaused.value){void retrySync();return;}paused.value=!paused.value;held.clear();queued=[];clock=0;accumulator=0;cancelAnimationFrame(frame);if(!paused.value){canvas.value?.focus();frame=requestAnimationFrame(loop);}}
