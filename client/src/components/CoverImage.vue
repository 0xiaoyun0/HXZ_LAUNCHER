...
function loaded(e:Event){const image=e.target as HTMLImageElement;natural.value={width:image.naturalWidth||1,height:image.naturalHeight||1};}
...
<template><div ref="root" class="cover-surface" :class="{editable}" @pointerdown="down" @pointermove="move" @pointerup="drag=null" @pointercancel="drag=null" @lostpointercapture="drag=null"><img :src="src" alt="实例头图" :style="imageStyle" draggable="false" @load="loaded"/></div></template>
<style scoped>.cover-surface{position:absolute;inset:0;overflow:hidden}.cover-surface img{position:absolute;max-width:none;user-select:none;pointer-events:none}.cover-surface.editable{touch-action:none;cursor:grab}.cover-surface.editable:active{cursor:grabbing}</style>
