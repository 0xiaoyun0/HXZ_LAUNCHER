...
0.3.0：公网社区恢复、GitHub 日志与自动安装更新、可恢复的整合包安装、MOD管理、固定服务器、全窗口背景与简化首页。详见 [更新日志](CHANGELOG.md) 和 [0.3.0 交付说明](docs/0.3.0开发与交付.md)。
...
- 通知公告页内分设原版生存群组、模组一服、模组二服入口；管理员发布/删除，自动刷新。
- 有界日志、导出、任务取消、游戏结束操作。

## 玩家使用

1. 打开 `release/0.3.0/client` 中的安装程序，或解压客户端便携包运行“便携启动.bat”。管理员的独立服务端包位于 `release/0.3.0/server`。
...
7. 使用已部署的幻想镇社区服务，或让管理员运行独立社区服务。客户端设置中填写地址并检查连接，再进入聊天大厅；公告无需登录即可读取，发言/语音需要皮肤站角色。
...
详见 [部署与接入.md](docs/部署与接入.md)。生产社区默认地址为 `https://qqbot.hxzmc.top`；HXZ UP 地址由实例配置提供，不内置未知生产域名。
...
- 本版提供独立 Minecraft/Forge/NeoForge/Fabric/Quilt 安装、Modrinth 整合包入口及本地 MOD 管理，不包含微软账号登录和在线模组市场。HXZ UP 继续管理游戏文件更新策略，electron-updater 负责启动器自身更新；两者与社区服务升级互相独立。
...
许可证与随包组件说明见 [第三方组件.md](docs/第三方组件.md)。

## 版本管理

仓库 main 保存当前源码，v0.2.2 起提供完整源码标签。v0.1.0、v0.2.0、v0.2.1 的早期源码快照未保留，这些标签记录原始产物清单，Release 提供对应原发布安装包，不能将其当作旧版源码快照。GitHub 会移除中文附件名称，公开附件改用 HXZ-Launcher / HXZ-Community，二进制内容保持原版，公开 SHA256SUMS.txt 对应公开文件名。

发布0.3.0以上版本时更新客户端与服务端版本，构建并整理两个独立发布包，创建 v版本号 标签及 GitHub Release 草稿。上传客户端安装程序、blockmap、latest.yml、便携包、服务端包及 SHA256SUMS，核对后发布正式Release。禁止把部署后的data/.env/profile打入公开附件。自动更新读取正式Release，不能先发布空Release再补附件。
