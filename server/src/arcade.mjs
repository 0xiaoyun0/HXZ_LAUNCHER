...
export function createArcade({db,auth,admin,body,send,limit,broadcast}){
  ...
  const runs=new Map();
  const board=game=>db.prepare('SELECT uid,name,score,updated FROM arcade_scores WHERE game=? ORDER BY score DESC,updated ASC LIMIT 50').all(game);
  return async(req,res,url)=>{
    ...
    if(!GAMES.some(g=>g.id===game))throw Error('小游戏不存在');
    ...
    if(req.method==='GET'){send(res,200,{items:board(game)});return true;}
    ...
    if(url.pathname.endsWith('/start')){
      ...
      if(run.result){send(res,200,{score:run.result.score,items:board(game)});return true;}
      if(Date.now()-run.started>20*60000||input.ticks/30*1000>Date.now()-run.started+2500)throw Error('游戏时长无效');
      const result=replay(game,run.seed,input.ticks,input.inputs);run.result={score:result.score};
      ...
      broadcast({type:'arcade-ranking',game});send(res,200,{score:result.score,items:board(game)});
